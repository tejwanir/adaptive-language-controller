...
# 2: "keep moving away"
# 3: "move towards" 
# 4: "keep moving inwards"


def get_move(previous_action,a):
    print(previous_action, a)
    ...
    if previous_action == 3:
        return 4
    if previous_action == 2: # based on a, return 3 or 4
        if a[0] < 0.2:
            return 2
        else:
            return 3
...
def solve():
    ...
    with open("knn_db.json", "r") as f:
        ...
        for i in db:
            if "keep" in i["phrase"] and (" in" in i["phrase"] or "forward" in i["phrase"]):
                phrases[4].append(i)
            elif ("far" in i["phrase"] or "keep" in i["phrase"]) and (" away" in i["phrase"] or " out" in i["phrase"]):
                phrases[2].append(i)
            elif "towards" in i["phrase"] and (" me" in i["phrase"] or " myself" in i["phrase"]):
                phrases[1].append(i)
            elif ("forward" in i["phrase"] or " away" in i["phrase"]) and " you" in i["phrase"]:
                phrases[1].append(i)
            elif "towards" in i["phrase"] and " you" in i["phrase"]:
                phrases[3].append(i)
            elif ("further" in i["phrase"]):
                phrases[3].append(i)
    ...
    while True:
        ...
        if (
            ...
            if len(next_phrase)!=0:
                if tts_player.ready.is_set():
                    tts_player.put_text(next_phrase)
            ...
            try:
                if start_pos[0] == -10:
                    start_pos = poses[0]["WristLeft"]
                    last_pos=start_pos
                    last_pos[0]-=0.1

                if poses[0]["WristLeft"][1] < start_pos[1]-0.12:
                    next_phrase = "Please move your hand down"
                    last_pos=poses[0]["WristLeft"]
                    continue
                if poses[0]["WristLeft"][1] > start_pos[1]+0.12:
                    next_phrase = "Please move your hand up"
                    last_pos=poses[0]["WristLeft"]
                    continue
                

                if previous_action == 2 and poses[0]["WristLeft"][0] < last_pos[0]:
                    next_phrase="Move the other way, leftward"
                    last_pos=poses[0]["WristLeft"]
                    continue
                if previous_action == 4 and poses[0]["WristLeft"][0] > last_pos[0]:
                    next_phrase="Move the other way, rightward"
                    last_post=poses[0]["WristLeft"]
                    continue
                
                last_pos = poses[0]["WristLeft"]

                previous_action = get_move(previous_action, np.array(poses[0]["WristLeft"] - start_pos))
                phrase = random.choice(phrases[previous_action])["phrase"]
                next_phrase = phrase
            except ValueError:
                pass
