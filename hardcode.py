...
def real_time_knn():
    ...
    while True:
        ...
        theta = 0
        try:
            wl = np.array(poses[0]["WristLeft"])
            sl = np.array(poses[0]["ShoulderLeft"])
            el = np.array(poses[0]["ElbowLeft"])
            v1=wl-el
            v2=sl-el
            theta=np.arccos(np.dot(v1,v2)/np.linalg.norm(v1)/np.linalg.norm(v2))
        except KeyError:
            theta = np.pi*2/3
        #gets the current position in pos_now
        if start_pos[0] == -10:
            start_pos = pos_now
        ...
        if (
            ...
            ############
            print(pos_now[0],pos_now[1],pos_now[2])


            if theta < 6*np.pi/12:
                print("Straighten")
                phrase = "Please straighten your arm"
                if tts_player.ready.is_set():
                    tts_player.put_text(phrase)
                    tts_player.ready.clear()
            elif theta > 10*np.pi/12:
                print("Relax")
                phrase = "Please relax your arm"
                if tts_player.ready.is_set():
                    tts_player.put_text(phrase)
                    tts_player.ready.clear()
            elif pos_now[1] < start_pos[1]-0.09:
                print("Move Down")
                phrase = "Please move your hand down"
                if tts_player.ready.is_set():
                    tts_player.put_text(phrase)
                    tts_player.ready.clear()
            elif pos_now[1] > start_pos[1]+0.09:
                print("Move Up")
                phrase = "Please move your hand up"
                if tts_player.ready.is_set():
                    tts_player.put_text(phrase)
                    tts_player.ready.clear()
            #elif pos_now[2] > start_pos[2]+0.15:
            #    print("Move Inward")
            #    phrase = "Please move your hand away from the camera"
            #    if tts_player.ready.is_set():
            #        tts_player.put_text(phrase)
            #        tts_player.ready.clear()
            #elif pos_now[2] < start_pos[2]-0.15:
            #    print("Move Outward")
            #    phrase = "Please move your hand towards the camera"
            #    if tts_player.ready.is_set():
            #        tts_player.put_text(phrase)
            ############
                ...
                v = pos_now - pos_past   # This is direction based? not position based?  This looks at the direction the hand is moving and generates words based on that
                try:
                    if abs(pos_now[0]-start_pos[0]) < 0.1:
                        print("Move Right")
                        phrase = "Please move your hand towards your side"
                        if tts_player.ready.is_set():
                            tts_player.put_text(phrase)
                            tts_player.ready.clear()
                    elif abs(pos_now[0]-(start_pos[0]+0.4)) < 0.1:
                        print("Move Left")
                        phrase = "Please move your hand towards my side"
                        if tts_player.ready.is_set():
                            tts_player.put_text(phrase)
                            tts_player.ready.clear()
                    print(v[0],v[1],v[2])
